...
export default function ShopPage() {
  ...
  const allBrands = useMemo(() => {
    ...
    return Array.from(brands.entries()).map(([name, count]) => ({
      name,
      count,
    }));
  }, [allProducts]);
  ...
  const filtered = useMemo(() => {
    ...
    if (filters.brands.length > 0)
      result = result.filter(
        (p) => p.brand && filters.brands.includes(p.brand.name)
      );
    if (
  ...
  const toggleBrand = (name: string) =>
    setFilters((f) => ({
      ...f,
      brands: f.brands.includes(name)
        ? f.brands.filter((x) => x !== name)
        : [...f.brands, name],
    }));
          ...
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                setFilters((f) => ({
                  ...f,
                  minPrice: Math.max(priceRange.min, Number(e.target.value)),
                }))
              }
              min={priceRange.min}
        ...
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between py-1">
            ...
            <button
              ...
              onClick={() =>
                setFilters((f) => ({ ...f, freeShipping: !f.freeShipping }))
              }
            >
              <span
                className={`absolute top-0.75 left-0.75 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${filters.freeShipping ? "translate-x-[1.1rem]" : ""}`}
              />
            </button>
          </div>
          <div className="flex items-center justify-between py-1">
            ...
            >
              <span
                className={`absolute top-0.75 left-0.75 w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${filters.inStock ? "translate-x-[1.1rem]" : ""}`}
              />
            </button>
          </div>
        </div>
  ...
  return (
    <div className="container px-4 sm:px-6 lg:px-8">
      <div className="py-6 pb-16 lg:pb-16">
        ...
        {activeFilterTags.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap mb-4">
            {activeFilterTags.map((tag, i) => (
              <button
                key={i}
                className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full bg-primary text-primary-foreground text-[11px] font-semibold border-none cursor-pointer hover:opacity-85 transition-opacity"
                onClick={tag.clear}
              >
                {tag.label}
                <X size={12} />
            ...
            <button
              className="text-[11px] font-semibold text-red-500 bg-transparent border-none cursor-pointer px-2 py-1 underline"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[260px_1fr] gap-6">
          ...
          <aside className="hidden lg:flex flex-col border border-border rounded-xl bg-card overflow-hidden h-fit sticky top-28">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
              ...
              {activeFilterTags.length > 0 && (
                <button
                  className="text-[11px] text-red-500 bg-transparent border-none cursor-pointer font-semibold"
                  onClick={clearAllFilters}
                >
                  Reset All
                </button>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-3 sm:gap-4 lg:gap-5">
            {filtered.length === 0 ? (
              <div className="col-span-full flex flex-col items-center justify-center py-16 text-center gap-3">
                ...
                <button
                  className="text-[11px] font-semibold text-red-500 bg-transparent border-none cursor-pointer underline"
                  onClick={clearAllFilters}
                >
                  Clear All Filters
                </button>
        ...
        {mobileOpen && (
          <div
            className="fixed inset-0 bg-black/40 z-50 flex lg:hidden"
            onClick={() => setMobileOpen(false)}
          >
            <div
              className="w-80 max-w-[85vw] h-full bg-card overflow-y-auto flex flex-col"
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-border">
                ...
                <button
                  className="flex items-center justify-center w-8 h-8 rounded border border-border bg-transparent text-foreground cursor-pointer"
                  onClick={() => setMobileOpen(false)}
                >
                  <X size={16} />
                </button>
              ...
              <div className="px-5 py-4 border-t border-border">
                <button
                  className="w-full py-3 rounded-lg bg-primary text-primary-foreground text-sm font-bold cursor-pointer hover:opacity-90 transition-opacity border-none"
                  onClick={() => setMobileOpen(false)}
                >
