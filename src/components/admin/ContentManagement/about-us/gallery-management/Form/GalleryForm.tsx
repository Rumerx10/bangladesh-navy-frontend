...
import { GallerySchemaForm } from "../Schema/gallerySchema";
import GalleryItemsField from "./GalleryItemsField";

interface GalleryFormProps {
  isEditMode?: boolean;
  onSubmit: (data: GallerySchemaForm) => void;
...
const CategoriesEditor = ({ disabled }: { disabled?: boolean }) => {
  const { setValue, watch } = useFormContext<GallerySchemaForm>();
...
const GalleryForm = ({
  ...
  const { handleSubmit } = useFormContext<GallerySchemaForm>();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-6">
      ...
      <div className="border border-light-silver rounded-lg p-6 sm:p-8 bg-white">
        ...
        <div className="flex flex-col gap-y-6 mt-6">
          ...
          <div>
            ...
            <ControlledInputField
              name="subTitle"
              placeholder="Glimpses of Bangladesh Navy operations"
              className="bg-light shadow-none"
            />
          </div>
        </div>
      ...
      {/* Gallery Items */}
      <div className="border border-light-silver rounded-lg p-6 sm:p-8 bg-white">
        <SectionHeader label="Gallery Items" />
        <div className="mt-6">
          <GalleryItemsField name="galleryItems" />
        </div>
      </div>
