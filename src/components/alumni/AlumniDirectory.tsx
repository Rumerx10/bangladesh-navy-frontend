...
import AlumniCourseGroupCard from "./AlumniCourseGroupCard";
...
const AlumniDirectory = () => {
  ...
  const { groups, isLoading } = useAlumniTree();

  // The filter bar is driven by the tree itself, so a course with no roster
  // never shows up as an empty pill.
  ...
  const visible = useMemo(
    () => filterAlumniGroups(groups, { courseId, search }),
    [groups, courseId, search]
  );
          ...
          <div className="mt-4">
            {isLoading ? (
              ...
              <div className="space-y-4">
                {visible.map((group) => (
                  <AlumniCourseGroupCard
                    key={group.id}
                    group={group}
                    isOpen={isSearching || openIds.includes(group.id)}
